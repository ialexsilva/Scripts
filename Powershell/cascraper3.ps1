...
function docheck ($property)
{
    ...
    if ($property.count -le $property.pcount)
    {
     ...
     Write-Output "$($property.code) checked at $(get-date) count $($property.count)" | Out-File C:\cascraper3.log -Append ascii    
    }
    else
    {
     $property.pcount = $property.count
     sendemail $property.techs $site.ParsedHtml.body.innerText
     Write-Output "$($property.code) checked at $(get-date) email sent $($property.count)" | Out-File C:\cascraper3.log -Append ascii
    }  
